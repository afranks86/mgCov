...
covarianceBiplot <- function(Vsub, Osamps, omegaSamps, s2samps, to_plot=1:nrow(s2samps), nlabeled=20) {
  ...
  p <- ggplot(as_tibble(Vsub[, 1:2]), colnames=c("V1", "V2")) +
    ...
    theme_bw() + xlim(xlimits) + ylim(ylimits)
  ...
  p <- p + geom_ellipse(data=ellipse_tibble,
                        aes(x0 = 0, y0 = 0, a = sd_max*2*max(xlimits)*0.25, b = sd_min*2*max(xlimits)*0.25, angle = angle, col=type), size=1.1)

  ## 50% contour
  p <- p + geom_ellipse(data=ellipse_tibble,
                        aes(x0 = 0, y0 = 0, a = sd_max*0.67*max(xlimits)*0.25, b = sd_min*0.67*max(xlimits)*.25, angle = angle, col=type), size=1.1)
